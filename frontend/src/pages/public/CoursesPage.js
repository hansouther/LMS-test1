...
export default function CoursesPage() {
  ...
  useSeo({
    title: "Katalog Kursus — Binara LMS",
    description: "Jelajahi kursus interaktif Binara LMS: video pembelajaran, latihan soal bernilai, dan Try Out CBT. Daftar gratis untuk mulai belajar.",
  });
      ...
      <section className="bg-[#0A1128] text-white py-16">
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/10 text-white px-4 py-1.5 text-xs font-semibold">
            <Sparkles className="h-3.5 w-3.5 text-[#C9A227]" /> Katalog Kursus
          </span>
          ...
          <Link to="/register" className="inline-flex items-center gap-2 mt-6 rounded-full bg-[#C9A227] hover:bg-[#A9871C] text-[#0A1128] font-semibold px-6 py-3 transition-colors duration-200" data-testid="courses-hero-cta">
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-8" data-testid="public-courses">
              ...
              {courses.map((c) => (
                <div key={c.id} className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden flex flex-col hover:-translate-y-1 transition-transform duration-200" data-testid={`public-course-${c.id}`}>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <Link to={`/kursus/${c.id}`}><h3 className="mt-1 font-semibold text-[#0A1128] hover:text-[#0E7490] transition-colors duration-200">{c.title}</h3></Link>
                    <p className="mt-2 text-sm text-[#475569] line-clamp-3 flex-1">{c.description}</p>
                    <div className="mt-4 flex items-center justify-between">
                      <span className="font-head font-bold text-[#0E7490]">{formatRupiah(parseInt(c.price || 0, 10))}</span>
                      <Link to={`/kursus/${c.id}`}><span className="inline-flex items-center gap-1.5 rounded-full bg-[#0E7490] hover:bg-[#0B5C74] text-white text-sm font-medium px-4 py-2 transition-colors duration-200" data-testid={`public-course-cta-${c.id}`}>Lihat Detail <ArrowRight className="h-3.5 w-3.5" /></span></Link>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="mt-12 rounded-2xl bg-white border border-[#E2E8F0] p-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="h-12 w-12 rounded-xl bg-[#E6F5F8] text-[#0E7490] flex items-center justify-center"><GraduationCap className="h-6 w-6" /></div>
              <div>
            ...
            <Link to="/login"><span className="inline-flex items-center gap-2 rounded-full border border-[#CBD5E1] hover:bg-[#E6F5F8] hover:text-[#0E7490] text-[#475569] font-medium px-6 py-2.5 transition-colors duration-200" data-testid="courses-login-cta">Masuk Portal</span></Link>
          </div>
        </div>
      </section>
    </PublicShell>

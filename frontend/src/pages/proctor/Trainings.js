          ...
          {data.map((t) => (
            <div key={t.course_id} className="bg-white rounded-2xl border border-[#E2E8F0] p-5 flex flex-col" data-testid={`training-${t.course_id}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <span className="inline-block rounded-full bg-[#EEF2FF] text-[#4361EE] px-2.5 py-0.5 text-[11px] font-semibold">{t.subject}</span>
              ...
              <div className="mt-4 flex items-center gap-2 text-sm text-[#475569]">
                <Users className="h-4 w-4 text-[#4361EE]" /> <span className="font-semibold text-[#0A1128]">{t.participants}</span> siswa mengikuti
              </div>
              <div className="mt-3 pt-3 border-t border-[#F1F5F9] flex flex-wrap gap-1.5">
                {t.students.slice(0, 8).map((name, i) => (
                  <span key={i} className="inline-flex items-center gap-1 rounded-full bg-[#F4F7FE] px-2.5 py-1 text-[11px] text-[#475569]" data-testid={`training-student-${t.course_id}-${i}`}>

      ...
      {loading ? <Loading /> : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <StatCard icon={BookOpen} label="Kursus Diikuti" value={data.enrollments} accent="#4361EE" testid="stat-enrollments" />
            <StatCard icon={ClipboardList} label="Try Out Selesai" value={data.completed_tryouts} accent="#FF9F1C" testid="stat-completed" />
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 rounded-2xl bg-[#0A1128] text-white p-8 relative overflow-hidden">
              <div className="absolute -top-16 -right-16 h-56 w-56 rounded-full bg-[#4361EE]/40 blur-3xl" />
              <div className="relative">
                ...
                <Link to="/student/tryouts"><Button className="mt-6 rounded-full bg-white text-[#0A1128] hover:bg-[#EEF2FF]" data-testid="go-tryouts">Mulai Sekarang <ArrowRight className="h-4 w-4" /></Button></Link>
              </div>
            </div>
            <div className="rounded-2xl bg-white border border-[#E2E8F0] p-6">
              ...
              <div className="mt-4 space-y-2">
                {[["Ruang Belajar", "/student/learning", Library], ["Katalog Kursus", "/student/courses", BookOpen], ["Jadwal Saya", "/student/schedule", CalendarCheck]].map(([l, to, Icon]) => (
                  <Link key={to} to={to} className="flex items-center gap-3 rounded-lg px-3 py-2.5 hover:bg-[#EEF2FF] text-[#475569] hover:text-[#4361EE] transition-colors duration-200">
              ...
              <div className="divide-y divide-[#E2E8F0]">
                {data.recent_attempts.map((a) => (
                  <div key={a.id} className="py-3 flex items-center justify-between" data-testid={`recent-attempt-${a.id}`}>
                    ...
                    <span className="font-mono2 text-lg font-bold" style={{ color: a.percentage >= 70 ? "#10B981" : a.percentage >= 50 ? "#FF9F1C" : "#EF4444" }}>{a.percentage}%</span>
                  </div>
                ))}
              </div>
